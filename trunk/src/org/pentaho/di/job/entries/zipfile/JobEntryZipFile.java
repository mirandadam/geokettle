...
public class JobEntryZipFile extends JobEntryBase implements Cloneable, JobEntryInterface {
  ...
  private boolean createParentFolder(String filename) {
    ...
    try {
      ...
      if (!parentfolder.exists()) {
        if (log.isDetailed())
          log.logDetailed(toString(), Messages.getString("JobEntryZipFile.CanNotFindFolder", "" + parentfolder.getName()));
        parentfolder.createFolder();
        if (log.isDetailed())
          log.logDetailed(toString(), Messages.getString("JobEntryZipFile.FolderCreated", "" + parentfolder.getName()));
      } else {
        if (log.isDetailed())
          log.logDetailed(toString(), Messages.getString("JobEntryZipFile.FolderExists", "" + parentfolder.getName()));
      }
      result = true;
    } catch (Exception e) {
  ...
  public boolean processRowFile(Job parentJob, Result result, String realZipfilename, String realWildcard, String realWildcardExclude,
    ...
    if (realZipfilename != null && orginExist) {
      ...
      try {
        ...
        // After Zip, Move files..User must give a destination Folder
        {
          ...
          if (fileList.length == 0) {
            resultat = false;
            log.logError(toString(), Messages.getString("JobZipFiles.Log.FolderIsEmpty", localSourceFilename));
          } else if (!checkContainsFile(localSourceFilename, fileList, isSourceDirectory)) {
            resultat = false;
            log.logError(toString(), Messages.getString("JobZipFiles.Log.NoFilesInFolder", localSourceFilename));
          } else {
            if (ifzipfileexists == 0 && Fileexists) {
              ...
              if (!renameOk) {
                log.logError(toString(), Messages.getString("JobZipFiles.Cant_Rename_Temp1.Label") + fileZip.getAbsolutePath()
                    + Messages.getString("JobZipFiles.Cant_Rename_Temp2.Label") + tempFile.getAbsolutePath()
            ...
            if (log.isBasic())
              log.logBasic(toString(), Messages.getString("JobZipFiles.Log.TotalZippedFiles", "" + zippedFiles.length));
            // Delete Temp File
            if (tempFile != null) {
              tempFile.delete();
            }
              ...
              for (int i = 0; i < zippedFiles.length; i++) {
                if (zippedFiles[i] != null) {
                  ...
                  if (afterzip == 1) {
                    ...
                    if (!deleted) {
                      resultat = false;
                      log.logError(toString(), Messages.getString("JobZipFiles.Cant_Delete_File1.Label") + localSourceFilename + Const.FILE_SEPARATOR + zippedFiles[i]
                          + Messages.getString("JobZipFiles.Cant_Delete_File2.Label"));

                    }
                    // File deleted
                    if (log.isDebug())
                      log.logDebug(toString(), Messages.getString("JobZipFiles.File_Deleted1.Label") + localSourceFilename + Const.FILE_SEPARATOR + zippedFiles[i]
      ...
      if (!orginExist)
        log.logError(toString(), Messages.getString("JobZipFiles.No_FolderCible_Defined.Label", localSourceFilename));
    }
    // return a verifier
    return resultat;
  }
  ...
  public Result execute(Result previousResult, int nr, Repository rep, Job parentJob) {
        ...
        try {
          ...
          if (moveToDirectory.exists()) {
            if (moveToDirectory.getType() == FileType.FOLDER) {
              if (log.isDetailed())
                log.logDetailed(toString(), Messages.getString("JobZipFiles.Log.MoveToFolderExist", realMovetodirectory));
            } else {
              SanityControlOK = false;
            ...
            if (log.isDetailed())
              log.logDetailed(toString(), Messages.getString("JobZipFiles.Log.MoveToFolderNotNotExist", realMovetodirectory));
            if (createMoveToDirectory) {
              moveToDirectory.createFolder();
              if (log.isDetailed())
                log.logDetailed(toString(), Messages.getString("JobZipFiles.Log.MoveToFolderCreaterd", realMovetodirectory));
            } else {
              SanityControlOK = false;
    ...
    if (isfromprevious) {
      if (log.isDetailed())
        log.logDetailed(toString(), Messages.getString("JobZipFiles.ArgFromPrevious.Found", (rows != null ? rows.size() : 0) + ""));
    }
